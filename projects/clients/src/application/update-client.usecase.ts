  ...
  initSubscriptions(): void {
    ...
    execute(client: Iclient, modal: ModalComponent): void {
      this.subscriptions.add(
        this._serviceUpdate.execute(client)
          .pipe(
            tap((user) => {
              const allClients = this._state.clientState.clients.valueState();
              const newClient = allClients.map(u => u.id === user.id ? user : u);
    ...
    selectClient(id: number): void {
      const clientActual = this._state.clientState.clients.valueState().find(u => u.id === id);
